...
import ConfirmationDialog from "../components/ConfirmationDilog"; // keep file name as in your project
...
// 🔹 helper to extract ID/name safely from different shapes
const getPartyId = (p = {}) =>
  p.PartyId || p.LedgerId || p.Id || p.id || p.partyId || "";
...
const getCompanyId = (c = {}) =>
  c.CompanyId || c.LedgerId || c.Id || c.id || c.comp_id || "";
...
const getFunctionId = (f = {}) =>
  f.FunctionId || f.Id || f.id || f.func_id || "";
...
function NewEnquiry() {
  ...
  useEffect(() => {
    ...
    setFormData((prev) => ({
      ...
      bookingToDate: editingEnquiry.FunctionTo
        ...
        : prev.bookingToDate,
      partyName: editingEnquiry.PartyName || prev.partyName,
      partyId: editingEnquiry.PartyId || prev.partyId,
      companyName: editingEnquiry.Company || prev.companyName,
      companyId: editingEnquiry.CompanyId || prev.companyId,
      functionName: editingEnquiry.Function || prev.functionName,
      functionId: editingEnquiry.FunctionId || prev.functionId,
    }));
  ...
  useEffect(() => {
    ...
    setFormData((prev) => {
      ...
      if (location.state.selectedParty) {
        ...
        updates.partyName = getPartyName(party);
        updates.partyId = getPartyId(party);
        hasUpdates = true;
      }

      if (location.state.selectedCompany) {
        const company = location.state.selectedCompany;
        updates.companyName = getCompanyName(company);
        updates.companyId = getCompanyId(company);
        hasUpdates = true;
      }

      if (location.state.selectedFunction) {
        const func = location.state.selectedFunction;
        updates.functionName = getFunctionName(func);
        updates.functionId = getFunctionId(func);
        hasUpdates = true;
      }
  ...
  const handleToggleAttendedBy = useCallback(() => {
    setShowOtherAttendedBy((prev) => {
      ...
      if (next) {
        ...
        setFormData((prevForm) => ({ ...prevForm, attendedBy: "" }));
      }
      return next;
    });
  }, [formData.attendedBy]);
  ...
  // Handle form submission – uses SAME API as NewBooking (submitBooking)
  const handleSubmit = useCallback(
    ...
    //   const gross = 0;
      ...
      const requestBody = {
        ...
        // Company / Function
        comp_id: formData.companyId || "",
        comp_name: formData.companyName || "",
        quot_status_id: "", // no status for enquiry
        ...
        single_event: "1",
        invoice_flag: "0", // enquiry never makes invoice here
        bill_id: "0",
        ...
        // Flags → IMPORTANT for enquiry
        enquiry: 1,
        AddedFrom: "E",
      };

      console.log("📦 Submitting enquiry payload to bookingApi.submitBooking:", requestBody);
    ...
    [formData, validateForm, navigate, editingEnquiryId, isEditMode]
  );
