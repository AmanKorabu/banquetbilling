...
const DemoBillList = () => {
    ...
    try {
      ...
      if (data && data.result && Array.isArray(data.result)) {
        const transformedBills = data.result.map((bill, index) => ({
          ...
          bill_id: bill.InvoiceId || "N/A",
          quotation_no: bill.QuotationNo || `Q${bill.QuotationId || index + 1}`,
          party_name: bill.PartyName || "N/A",
          function_name: bill.FunctionName || "N/A",
          date: bill.QuotationDate || bill.CreatedDate || "N/A",
          quotation_date: bill.QuotationDate || "N/A",
          function_date: bill.QuotationDate || "N/A",
          amount: bill.BillAmount || "0",
          received_amount: bill.ReceivedAmount || "0",
          discount: bill.Discount || "0",
          tds: bill.TDS || "0",
          balance: (parseFloat(bill.BillAmount || 0) - parseFloat(bill.ReceivedAmount || 0)).toFixed(2),
          status: bill.Status,
          status_id: bill.StatusId || "",
          billing_company: bill.BillingCompany || "N/A",
          mobile: bill.Mobile || bill.Contact || "N/A",
          guest_count: bill.GuestCount || bill.Pax || bill.MaxPax || "0",
        ...
        setBills([]);
        toast.info("No quotations found for selected dates", { toastId: 'info-bill' });
      }
    } catch (error) {
      console.error("❌ Error fetching bills:", error);
      toast.error("Failed to load quotations", { toastId: 'failed-bill' });
      setBills([]);
    } finally {
      setLoading(false);
    }
  ...
  const filteredBills = useMemo(() => {
    return bills.filter(bill => {
      const matchesSearch =
        ...
        bill.quotation_no.toLowerCase().includes(searchTerm.toLowerCase()) ||
        bill.function_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        bill.billing_company.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (bill.mobile && bill.mobile.includes(searchTerm));
  ...
  const financialSummary = useMemo(() => {
    const summary = {
      totalQuotations: filteredBills.length,
      totalQuotationAmount: 0,
      totalReceivedAmount: 0,
      totalDiscount: 0,
      totalTDS: 0,
      totalBalance: 0,
      averageQuotationAmount: 0,
      statusBreakdown: {},
    ...
    filteredBills.forEach(bill => {
      ...
      const received = parseFloat(bill.received_amount) || 0;
      const discount = parseFloat(bill.discount) || 0;
      const tds = parseFloat(bill.tds) || 0;
      const balance = parseFloat(bill.balance) || 0;

      summary.totalQuotationAmount += amount;
    ...
    summary.averageQuotationAmount = summary.totalQuotations > 0
      ? summary.totalQuotationAmount / summary.totalQuotations
      : 0;
  ...
  const handleDeleteConfirm = async () => {
    if (!deletePopup.bill || deletePopup.reason.length < 3) return;

    setDeletePopup(prev => ({ ...prev, loading: true }));

    try {
      await fetch(`/banquetapi/delete_or_active_quot.php?quot_id=${deletePopup.bill.quot_id}&action=delete&cancel_reason=${encodeURIComponent(deletePopup.reason)}`, {
        method: 'POST'
      });

      toast.success("Quotation deleted successfully!", { toastId: 'delete-success' });
      closeDeletePopup();
      fetchBills(); // Refresh the list
    } catch (error) {
      console.error("❌ Delete error:", error);
      toast.error("Failed to delete quotation", { toastId: 'delete-failed' });
      setDeletePopup(prev => ({ ...prev, loading: false }));
    }
  };

  // Handle New Bill (Create mode)
  const handleNewBill = () => {
  ...
  // Handle Bill Modify (Edit existing quotation)
  const handleBillModify = (bill) => {
    const hotelId = localStorage.getItem("hotel_id");
    ...
    navigate("/new-booking", {
      state: {
        ...
        quotationMeta: {
          quot_id: bill.quot_id,
          hotel_id: hotelId,
          quotation_no: bill.quotation_no,
  ...
  const getStatusConfig = (status) => {
    ...
    switch (status.toLowerCase()) {
      ...
      case 'waitlisted':
        return { color: '#f59e0b', bgColor: '#fffbeb', borderColor: '#fed7aa' };
      case 'tentative':
        return { color: '#8b5cf6', bgColor: '#faf5ff', borderColor: '#ddd6fe' };
      default:
    ...
    <>
      ...
      <div className="demo-bill-list">
          ...
          <div className="financial-summary-modal">
            <div className="financial-summary-content">
              ...
              <div className="summary-grid">
                <div className="summary-card total-amount">
                  ...
                  <div className="summary-info">
                    ...
                    <div className="summary-amount">
                      ₹{formatCurrency(financialSummary.totalQuotationAmount)}
                    </div>
                    <div className="summary-subtext">
                      Across {financialSummary.totalQuotations} quotations
                    </div>
                  </div>
                </div>
                ...
                <div className="summary-card received-amount">
                  ...
                  <div className="summary-info">
                    ...
                    <div className="summary-subtext">
                      {((financialSummary.totalReceivedAmount / financialSummary.totalQuotationAmount) * 100 || 0).toFixed(1)}% of total
                    </div>
                  </div>
                </div>
                ...
                <div className="summary-card balance-amount">
                  ...
                  <div className="summary-info">
                    ...
                    <div className="summary-subtext">
                      {((financialSummary.totalBalance / financialSummary.totalQuotationAmount) * 100 || 0).toFixed(1)}% pending
                    </div>
                  </div>
                </div>
                ...
                <div className="summary-card discount-amount">
                  ...
                  <div className="summary-info">
                    ...
                    <div className="summary-subtext">
                      {((financialSummary.totalDiscount / financialSummary.totalQuotationAmount) * 100 || 0).toFixed(1)}% of total
                    </div>
                  </div>
                </div>
                ...
                <div className="summary-card average-amount">
                  ...
                  <div className="summary-info">
                    <h3>Average Quotation</h3>
                    <div className="summary-amount">
                      ₹{formatCurrency(financialSummary.averageQuotationAmount)}
                    </div>
                    <div className="summary-subtext">
                      Per quotation
                    </div>
                ...
                <div className="status-list">
                  {Object.entries(financialSummary.statusBreakdown).map(([status, data]) => (
                    <div key={status} className="status-item">
                      ...
                      <span className="status-count">{data.count} quotations</span>
              ...
              <div className="summary-footer">
                <p>
                  Showing financial data for <strong>{filteredBills.length}</strong> quotations
          ...
          <div className="header-content">
            <div className="header-main">
              <div className="header-title">
                <h1>Quotations</h1>
                <p>Manage banquet quotations and bookings</p>
              </div>
            ...
            <div className="header-stats">
              ...
              >
                <span className="stat-number">{statusCounts.waitlisted}</span>
              ...
              >
                <span className="stat-number">{statusCounts.tentative}</span>
            ...
            <div className="filter-main">
              <div className="date-filters">
                ...
                <div className="search-box">
                  <input
                    type="text"
                    placeholder="Search by name, quotation no, company..."
                    value={searchTerm}
                    ...
                    <div key={bill.id} className="bill-card">
                      <div className="card-header">
                        <div className="card-title-section">
                          <h3 className="quotation-no">{bill.quotation_no}</h3>
                          <span
                        ...
                        <div className="party-info">
                          <strong className="party-name2">{bill.party_name}</strong>
                          <span className="company-name">{bill.billing_company}</span>
                        </div>
                      </div>
                      ...
                      <div className="card-body">
                        <div className="function-details">
                          <div className="function-row">
                            ...
                            <span className="function-date">{formatDate(bill.function_date)}</span>
                            <span className="function-type">{bill.function_name}</span>
                          </div>
                        </div>

                        <div className="financial-summary">
                          <div className="amount-row main-amount">
                          ...
                          <div className="amount-details">
                            <div className="amount-column">
                              <div className="amount-item">
                                <span>Received:</span>
                                <span className="received">₹{formatCurrency(bill.received_amount)}</span>
                              </div>
                              <div className="amount-item">
                                <span>Discount:</span>
                            ...
                            <div className="amount-column">
                              <div className="amount-item">
                                <span>Balance:</span>
                                <span className={`balance ${parseFloat(bill.balance) > 0 ? 'pending' : 'paid'}`}>
                                  ₹{formatCurrency(bill.balance)}
                                </span>
                              </div>
                              <div className="amount-item">
                                <span>TDS:</span>
                                <span className="tds">₹{formatCurrency(bill.tds)}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                ...
                <table className="bills-table">
                  <thead>
                    <tr>
                      ...
                      <th className="quotation-no">Quotation No</th>
                      <th className="function-date">Function Date</th>
                      <th className="party-name2">Party Name</th>
                      <th className="received-amount">Received Amt</th>
                      <th className="discount">Discount</th>
                      <th className="tds">TDS</th>
                      <th className="quotation-amount">Quotation Amt</th>
                      <th className="balance">Balance</th>
                      <th className="company-name">Company Name</th>
                      <th className="status">Status</th>
                      <th className="quotation-date">Quotation Date</th>
                      ...
                      return (
                        <tr key={bill.id} className="bill-row">
                          ...
                          <td className="quotation-no">
                            <strong>{bill.quotation_no}</strong>
                          </td>
                          <td className="function-date">
                            {formatDate(bill.function_date)}
                          </td>
                          <td className="party-name2">
                            <div className="party-info">
                              <strong>{bill.party_name}</strong>
                            </div>
                          </td>
                          ...
                          <td className="balance">
                            <div className={`currency-cell ${parseFloat(bill.balance) > 0 ? 'balance-pending' : 'balance-paid'}`}>
                              <FaRupeeSign size={10} />
                              {formatCurrency(bill.balance)}
                            </div>
                          </td>
                          <td className="company-name">
                            {bill.billing_company}
                          </td>
                          <td className="status">
                            <span
                              className="status-badge"
        ...
        <div className="list-summary">
          <p>
            Showing <strong>{filteredBills.length}</strong> of <strong>{bills.length}</strong> quotations
            {statusFilter !== 'all' && ` • Filtered by: ${statusFilter}`}
            {searchTerm && ` • Search: "${searchTerm}"`}
            <span className="financial-totals">
              • Total: ₹{formatCurrency(financialSummary.totalQuotationAmount)}
        ...
        <style>{`
          ...
          .stat-card.total.active { background: #071936; color: white; }
          .stat-card.confirmed { border-left-color: #10b981; }
          .stat-card.confirmed.active { background: #003625ff; color: white; }
          .stat-card.waitlisted { border-left-color: #f59e0b; }
          .stat-card.waitlisted.active { background: #7f4400ff; color: white; }
          .stat-card.tentative { border-left-color: #8b5cf6; }
          .stat-card.tentative.active { background: #27006aff; color: white; }
          ...
          .tds { color: #8b5cf6; }
          .balance.pending { color: #ef4444; font-weight: 600; }
          ...
          @media (max-width: 767px) {
            ...
            .date-filter-group, .search-box, .date-controls {
              width: 100%;
            }
