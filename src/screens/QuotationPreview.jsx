...
const QuotationPreview = () => {
  useEscapeNavigate('/select-dashboard')
  ...
  const {
    ...
    bill_id,
    quotationId,
    received_amount: navReceivedAmount,
  ...
  }, [quot_id, bill_id, navigate]);

  const fetchQuotationData = async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `/banquetapi/print_preview.php?hotel_id=${hotel_id}&quot_id=${quot_id}&bill_id=${bill_id}`
      );
  ...
  const sharePDFViaWhatsApp = async (pdfBlob) => {
    ...
    const message = 
      ...
      `Amount: ₹${mainData.BillAmount}\n` +
      `Date: ${mainData.QuotationDate}\n` +
      `Quotation No: ${mainData.QuotationNo || quot_id}\n\n` +
    ...
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [pdfFile] })) {
      try {
        await navigator.share({
          files: [pdfFile],
          title: `Quotation - ${mainData.QuotationNo || quot_id}`,
          text: message
        });
    ...
    setTimeout(() => {
      ...
      downloadLink.download = `quotation_${mainData.QuotationNo || quot_id}.pdf`;
  ...
  const handleEdit = () => {
    ...
    navigate("/new-booking", {
      state: {
        ...
        quotationMeta: {
          hotel_id,
          quot_id: quot_id || mainData.QuotId || mainData.QuotationId,
  ...
  // Group items by subcategory (from BillPreview)
  ...
  // Format time to 12h format (from BillPreview)
  ...
  if (loading) {
    return (
      <div className="loading-container">
        <div className="loading-spinner">Loading Quotation Preview...</div>
      </div>
    );
  }
  ...
  if (!quotationData) {
    return (
      <div className="error-container">
        <div>No quotation data found</div>
  ...
  const mainData = quotationData.result[0];
  const events = quotationData.events || [];
  const firstEvent = events[0] || {};

  const allItems = events.flatMap(event => event.items_arr || []);
  const groupedItems = groupItemsBySubCategory(allItems);

  // Calculate totals with enhanced logic from BillPreview
  ...
  // 🔥 CRITICAL FIX: Get receipts from API response like BillPreview does
  ...
  // 🔥 CRITICAL FIX: Enhanced Advance Received calculation (same as BillPreview)
  const advanceReceived = (() => {
    // If we have receipts data, use the calculated total from receipts
  ...
  // Enhanced TDS calculation (from BillPreview)
  const tds = (() => {
    const raw = mainData.TDS ?? navTds ?? 0;
    const num = parseFloat(raw || 0);
    return isNaN(num) ? 0 : num;
  })();
  ...
  return (
    <div className="preview-container">
      ...
      {/* Quotation Content */}
      <div className="quotation-print" id="quotation-print">
        ...
        <div className="customer-details">
          <table className="details-table">
            <tbody>
              <tr>
                <td width="50%"><strong>Quotation No:</strong> {mainData.QuotationNo || quot_id}</td>
                <td width="50%"><strong>Quotation Date:</strong> {mainData.QuotationDate}</td>
              </tr>
              <tr>
                <td><strong>Billing Name:</strong> {mainData.BillingCompany}</td>
                <td><strong>Attended By:</strong> {mainData.AttendedByName}</td>
              </tr>
              <tr>
                <td><strong>Guest Name:</strong> {mainData.PartyName}</td>
                <td><strong>Guest Mobile:</strong> {mainData.PhoneNo}</td>
              </tr>
              <tr>
                <td><strong>Company Name:</strong> {mainData.CompanyName || 'RN Software and consultors'}</td>
                <td><strong>GST No.:</strong> {mainData.GSTNo || 'N/A'}</td>
              </tr>
              <tr>
                <td><strong>Address:</strong> {mainData.AddressLine1} {mainData.AddressLine2}</td>
                <td><strong>Venue:</strong> {firstEvent.VenueName}</td>
              </tr>
              <tr>
                <td><strong>Status:</strong> <span className="status">{mainData.Status}</span></td>
                <td><strong>Function:</strong> {mainData.FunctionName}</td>
              </tr>
              <tr>
                <td><strong>Min Pax:</strong> {firstEvent.MinPax}</td>
                <td><strong>Function Start:</strong> {mainData.QuotationDate} {formatTimeTo12h(mainData.FunctionTime)}</td>
              </tr>
              <tr>
                <td></td>
                <td><strong>Function End:</strong> {mainData.QuotationDateTo} {formatTimeTo12h(firstEvent.TimeTo)}</td>
              </tr>
            </tbody>
          </table>
        </div>

        {/* Items Table with Sub-Categories (Enhanced from BillPreview) */}
        <div className="items-section">
          <table className="items-table">
            <thead>
              <tr>
                <th width="60%" style={{ textAlign: 'left' }}>Item Name</th>
                <th width="13%" className="text-center">Qty</th>
                <th width="13%" className="text-center">Rate</th>
                <th width="14%" className="text-center">Amount</th>
              </tr>
            </thead>
            <tbody>
                ...
                <td width="40%" className="text-right">
                  <table className="amount-table">
                    <tbody>
                      ...
                      <tr>
                        <td><strong>Settlement Discount:</strong></td>
                        <td className="text-center">
                          ₹{settlementDiscount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </td>
                      </tr>
                      <tr>
        ...
        <div className="amount-words">
          <p><strong>Rupees:</strong> {convertNumberToWords(grandTotal)}</p>
        </div>

        {/* Tax Summary */}
        <div className="tax-summary">
          <p><strong>Tax Summary:</strong> {firstEvent.tax_arr?.[0]?.TaxSummary || 'CGST 9% 3,240.00, SGST 9% 3,240.00'}</p>
        </div>
        ...
        <div className="print-footer">
          <div className="signatures">
            <div className="jurisdiction">
              <p><strong>Subject to Jurisdiction:</strong> Kolhapur</p>
              <p><strong>Bank Details:</strong> XPRESS BANQUET, A/C No: XXXX XXXX XXXX, IFSC: XXXX</p>
            </div>
